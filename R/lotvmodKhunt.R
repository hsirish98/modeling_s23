...
#'  \emph{huntr} is a rate at which the predator will hunt the pray
...
lotvmodK_hunt = function(t, pop, pars) {
  with(as.list(c(pars,pop)), {
    ...
    dprey = ifelse(prey<0.2*rprey, rprey*(1-prey/K)*prey -  alpha*prey*pred, rprey*(1-prey/K)*prey -  alpha*prey*pred - huntr*prey)

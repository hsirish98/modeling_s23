...
k_o_boxplot <- ggplot(plot_df, aes(x = cut_interval(x=k_o, length=.01), y = conductance)) +
  ...
  theme_minimal()
v_boxplot <- ggplot(plot_df, aes(x = cut_interval(x=v, length=60), y = conductance)) +
  geom_boxplot(color = "gold") +
  labs(x= "Wind Speed", y="Conductance", color="Conductance")+
  theme_minimal()
height_boxplot <- ggplot(plot_df, aes(x = cut_interval(x=height, length=25), y = conductance)) +
